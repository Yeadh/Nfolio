...
class CSFramework_Option_background extends CSFramework_Options {
  ...
  public function output() {
    ...
    $button_title   = ( isset( $button_title ) ) ? $button_title : __( 'Upload', 'creek' );
    $frame_title    = ( isset( $frame_title  ) ) ? $frame_title  : __( 'Upload', 'creek' );
    $insert_title   = ( isset( $insert_title ) ) ? $insert_title : __( 'Use Image', 'creek' );

    echo '<div class="cs-field-upload">';
    echo '<input type="hidden" name="' . $this->element_name( '[id]' ) . '" value="' . $this->value['id'] . '" />';
